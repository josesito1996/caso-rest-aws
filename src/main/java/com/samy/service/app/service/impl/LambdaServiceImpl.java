...
import static com.samy.service.app.util.Contants.lambdaTestFileS3;
...
public class LambdaServiceImpl implements LambdaService {
  ...
  private String getEnviroment() {
    return enviroment.equals("dev") ? "" : "-prod";
  }
  ...
  public JsonObject invocarLambdaMail(LambdaMailRequest request) {
    ...
    try {
      ...
      InvokeRequest invokeRequest =
          new InvokeRequest().withFunctionName(lambdaMailNombre).withPayload(payLoad);
  ...
  public JsonObject enviarCorreo(LambdaMailRequestSendgrid request) {
    try {
      ...
      InvokeRequest invokeRequest =
          new InvokeRequest().withFunctionName(lambdaMailSenderNombre).withPayload(payLoad);
      InvokeResult result = awsLambda.invoke(invokeRequest);
      String ans = new String(result.getPayload().array(), StandardCharsets.UTF_8);
      JsonElement element = JsonParser.parseString(ans);
  ...
  @Override
  public JsonObject eliminarArchivo(LambdaFileRequest request) {
    try {
      log.info("Cuerp de la peticion para el Eliminar archivo : " + request);
      Gson gson = new Gson();
      String payLoad = gson.toJson(request);
      log.info("PayLoad {}", payLoad);
      InvokeRequest invokeRequest = new InvokeRequest()
          .withFunctionName(lambdaTestFileS3.concat(getEnviroment())).withPayload(payLoad);
      InvokeResult result = awsLambda.invoke(invokeRequest);
      String ans = new String(result.getPayload().array(), StandardCharsets.UTF_8);
      JsonObject jsonResponse = new JsonObject();
      jsonResponse.addProperty("status", true);
      jsonResponse.addProperty("response", ans);
      return jsonResponse;
    } catch (ServiceException e) {
      e.printStackTrace();
      log.error("Error al invocar lambda -> " + e.toString());
      ...
      jsonResponse.addProperty("response", "Error");
      return jsonResponse;
    }
  }
}
