...
public class ServiceUtils {
	...
	public static Integer nroOrdenEtapaActuacion(List<Actuacion> actuaciones) {
		return actuaciones.isEmpty() ? 0 : actuaciones.get(actuaciones.size() - 1).getEtapa().getNroOrden();
	}
	...
	public static FuncionarioDto funcionario(List<Actuacion> actuaciones) {
		...
		return funcionariosSize > 0 ? new FuncionarioDto(funcionario.getId(), funcionario.getDatosFuncionario())
				: new FuncionarioDto();
	}
	...
	public static int totalActuacionesCompletadasGeneral(List<Caso> casos, Boolean estado) {
		...
		for (Caso caso : casos) {
			...
			for (Actuacion actuacion : caso.getActuaciones()) {
				cantidad = cantidad
						+ (int) actuacion.getTareas().stream().filter(tarea -> tarea.isEstado() == estado).count();
				if (cantidad > 0) {
					totalCompletas++;
				}
			}
